...
#' Plot the PDF of the Cauchy Distribution
#'
...
#' @export
cauchy.pdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.pdf(main = "Cauchy Distribution", fns = c(qcauchy, pcauchy, dcauchy), params = params, limits = limits, probs = probs)
  do.call(plot.pdf, c(calc, ...))
}

#' Plot the CDF of the Cauchy Distribution
#'
...
#' @return A plot is produced. Invisibly returns `NULL`.
#' @export
cauchy.cdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.cdf(main = "Cauchy Distribution", fns = c(qcauchy, pcauchy, dcauchy), params = params, limits = limits, probs = probs)
