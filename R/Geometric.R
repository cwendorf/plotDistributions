...
#' Plot the PDF of the Geometric Distribution
#'
...
#'
#' @param params Optional list of parameters for the distribution (e.g., `prob`).
...
#' @export
geom.pdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), width = 0.8, ...) {
  calc <- calc.pdf(main = "Geometric Distribution", fns = c(qgeom, pgeom, dgeom), params = params, limits = limits, probs = probs)
  do.call(plot.pdf, c(calc, list(width = width), ...))
}

#' Plot the CDF of the Geometric Distribution
#'
#' Generates a plot of the cumulative distribution function (CDF) for the geometric distribution, with optional visual indicators of probability differences between specified limits or quantiles.
#'
#' @param params Optional list of parameters for the distribution (e.g., `prob`).
...
#' @return A plot is produced. Invisibly returns `NULL`.
#' @export
geom.cdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.cdf(main = "Geometric Distribution", fns = c(qgeom, pgeom, dgeom), params = params, limits = limits, probs = probs)
