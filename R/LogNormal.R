...
#'
#' @param params Optional list of parameters for the distribution (e.g., `meanlog`, `sdlog`).
...
#' @export
lnorm.pdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.pdf(main = "Log-Normal Distribution", fns = c(qlnorm, plnorm, dlnorm), params = params, limits = limits, probs = probs)
  do.call(plot.pdf, c(calc, ...))
}

#' Plot the CDF of the Log-Normal Distribution
#'
...
#' @return A plot is produced. Invisibly returns `NULL`.
#' @export
lnorm.cdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.cdf(main = "Log-Normal Distribution", fns = c(qlnorm, plnorm, dlnorm), params = params, limits = limits, probs = probs)
