...
#'
#' @param params Optional list of parameters for the distribution (e.g., `shape`, `rate`).
...
#' @export
gamma.pdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.pdf(main = "Gamma Distribution", fns = c(qgamma, pgamma, dgamma), params = params, limits = limits, probs = probs)
...
#' @return A plot is produced. Invisibly returns `NULL`.
#' @export
gamma.cdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.cdf(main = "Gamma Distribution", fns = c(qgamma, pgamma, dgamma), params = params, limits = limits, probs = probs)
