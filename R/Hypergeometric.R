...
#' Plot the PDF of the Hypergeometric Distribution
#'
...
#'
#' @param params Optional list of parameters for the distribution (e.g., `m`, `n`, `k`).
...
#' @export
hyper.pdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), width = 0.8, ...) {
  calc <- calc.pdf(main = "Hypergeometric Distribution", fns = c(qhyper, phyper, dhyper), params = params, limits = limits, probs = probs)
  do.call(plot.pdf, c(calc, list(width = width), ...))
}

#' Plot the CDF of the Hypergeometric Distribution
#'
...
#' @return A plot is produced. Invisibly returns `NULL`.
#' @export
hyper.cdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.cdf(main = "Hypergeometric Distribution", fns = c(qhyper, phyper, dhyper), params = params, limits = limits, probs = probs)
