...
#'
#' @param params Optional list of parameters for the distribution (e.g., `rate`).
...
#' @export
exp.pdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.pdf(main = "Exponential Distribution", fns = c(qexp, pexp, dexp), params = params, limits = limits, probs = probs)
...
#' @return A plot is produced. Invisibly returns `NULL`.
#' @export
exp.cdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.cdf(main = "Exponential Distribution", fns = c(qexp, pexp, dexp), params = params, limits = limits, probs = probs)
