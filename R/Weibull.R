...
#'
#' @param params Optional list of parameters for the distribution (e.g., `shape`, `scale`).
...
#' @export
weibull.pdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.pdf(main = "Weibull Distribution", fns = c(qweibull, pweibull, dweibull), params = params, limits = limits, probs = probs)
  do.call(plot.pdf, c(calc, ...))
}

#' Plot the CDF of the Weibull Distribution
#'
...
#' @return A plot is produced. Invisibly returns `NULL`.
#' @export
weibull.cdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.cdf(main = "Weibull Distribution", fns = c(qweibull, pweibull, dweibull), params = params, limits = limits, probs = probs)
