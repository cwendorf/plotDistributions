...
#' Plot the PDF of the Negative Binomial Distribution
#'
...
#' @export
nbinom.pdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), width = 0.8, ...) {
  calc <- calc.pdf(main = "Negative Binomial Distribution", fns = c(qnbinom, pnbinom, dnbinom), params = params, limits = limits, probs = probs)
  do.call(plot.pdf, c(calc, list(width = width), ...))
}

#' Plot the CDF of the Negative Binomial Distribution
#'
...
#' @return A plot is produced. Invisibly returns `NULL`.
#' @export
nbinom.cdf <- function(params = NULL, limits = c(NULL, NULL), probs = c(NULL, NULL), ...) {
  calc <- calc.cdf(main = "Negative Binomial Distribution", fns = c(qnbinom, pnbinom, dnbinom), params = params, limits = limits, probs = probs)
